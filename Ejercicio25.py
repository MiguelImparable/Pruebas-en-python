...
def Exercise_25():
    ...
    while True:
        try:
            ...
            x1 = list(range(Calles)); np.random.shuffle(x1)
            x2 = list(range(Carreras)); np.random.shuffle(x2)
            MalladoVial = []
            MalladoVial2 = []
            Propietarios = []
            ListaPropietarios = []
            ...
            CarrerasList = []
            VehiculosList = []
            LL = ['b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm','n', 'ñ', 'p', 'q', 'r', 's', 't', 'v', 'w', 'x', 'y', 'z']
            LV = ['a', 'e', 'i', 'o', 'u']
            NO = list(range(10))
            Puestos = ["Dos Puestos", "Cuatro Puestos", "Seis Puestos"]
            TipoVehiculo = ["Familiar", "Deportivo"]
            Lista = ["Calle", "Carrera"]
            EstSem = ["Amrillo", "Rojo", "Verde", "Intermitente", "Fuera de Servicio"]
            # Calles
            for i in range(Calles):
                MalladoVial.append(x1[i])
            for i in range(Calles):
                if MalladoVial[i] % 2 == 0:
                    CallesList.append("Calle: {} Oriente".format(MalladoVial[i]))
                else:
                    CallesList.append("Calle: {} Occidente".format(MalladoVial[i]))
            # Carreras
            for i in range(Carreras):
                MalladoVial2.append(x2[i])
            for i in range(Carreras):
                if MalladoVial2[i] % 2 == 0:
                    CarrerasList.append("Carrera: {} Norte".format(MalladoVial2[i]))
                else:
                    CarrerasList.append("Carrera: {} Sur".format(MalladoVial2[i]))
            # Nombres y Apellidos Propietarios del Auto
            for i in range(Habitantes):
                np.random.shuffle(LL); np.random.shuffle(LV)
                c1 = "{}{}{}{}{}{}".format(LL[0],LV[0],LL[1],LV[1],LL[2],LV[2])
                np.random.shuffle(LL); np.random.shuffle(LV)
                c1 += " {}{}{}{}{}{}".format(LL[0],LV[0],LL[1],LV[1],LL[2],LV[2])
                c1 = c1.capitalize()
            ...
            for i in range(len(Vehiculos)):
                if i < Limite:
                    b1 = (Propietarios[i], Vehiculos[i], Puestos[random.randint(0, 2)], TipoVehiculo[random.randint(0, 1)])
                    VehiculosList.append(b1)
            # Vehiculos
            for i in range(nvechiculos):
                np.random.shuffle(LL); np.random.shuffle(NO)
                c1 = LL[0] + LL[1] + LL[2] + str(NO[0]) + str(NO[1]) + str(NO[2])
                c1 = c1.capitalize()
                Vehiculos.append(c1)
            # Semaforos
            a = CallesList + CarrerasList; np.random.shuffle(a)
            for i in range(Calles):
                x = "Semaforo: {} Estado: {}".format(a[i], EstSem[random.randint(0, 4)])
